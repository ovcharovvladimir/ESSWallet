...
function formatDate (date) {
  return vreme.format(new Date(date), '16 Dec 2014 at 14:30')
}
  ...
  if (decimalsToKeep === undefined) {
    if (beforeDecimal === '0') {
      if (afterDecimal !== '0') {
        ...
        formatted = '0.' + afterDecimal + ' ESS'
      }
    } else {
      formatted = beforeDecimal + '.' + afterDecimal.slice(0, 3) + ' ESS'
    }
  } else {
    afterDecimal += Array(decimalsToKeep).join('0')
    formatted = beforeDecimal + '.' + afterDecimal.slice(0, decimalsToKeep) + ' ESS'
  }
  return formatted
}
